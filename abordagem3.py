...
def runDoc2Vec(lemmatized_tokens, window_size):
    ...
    model = Doc2Vec(tagged_data, window=window_size)  # Doc2Vec(documents=tagged_data, size=100, workers=4, window=3)
...
model_sum = np.empty((492,100)) #deixar 100?
for window in windows:
    model = runDoc2Vec(descriptions, window)
    model_sum = np.add(model_sum, model)
print(model_sum)
run_classification(model_sum, points)
