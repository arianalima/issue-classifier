...
def runDoc2Vec(lemmatized_tokens):
    tagged_data = [TaggedDocument(words=lemmatized_tokens, tags=[str(i)]) for i, _d in enumerate(lemmatized_tokens)]
    # model = Doc2Vec(tagged_data, workers=4, window=10, vector_size=500, dm=1)
    # model.train(tagged_data, total_examples=model.corpus_count, epochs=model.epochs)

    model = Doc2Vec(tagged_data, vector_size=400, window=10)  # Doc2Vec(documents=d2v_sentences, size=100, workers=4, window=3)
    model.train(tagged_data, total_examples=model.corpus_count, epochs=1500)
...
def read_dataset(path):
    ...
    descriptions = runDoc2Vec(lemmatized_tokens)
